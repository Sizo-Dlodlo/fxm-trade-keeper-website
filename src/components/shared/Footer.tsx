    ...
    <footer className="w-full pt-20 pb-10 bg-surface-container-lowest border-t border-surface-stroke/30">
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-gutter px-4 md:px-margin-desktop max-w-container-max mx-auto mb-12">
        ...
        <div className="col-span-2 flex flex-col gap-4">
          ...
          <p className="font-body text-body-md text-text-dimmed max-w-xs mt-2">
            Professional, local-first trading journal software built for
            disciplined traders.
          </p>
          <Link
            href="/support-the-project"
            className="mt-4 bg-transparent border border-primary text-primary px-4 py-2 rounded hover:bg-primary/10 transition-colors self-start font-mono text-label-mono flex items-center gap-2"
          >
        ...
        <div className="flex flex-col gap-3">
          <h4 className="font-display text-base text-on-surface mb-2">
            Product
          </h4>
          {footerLinks.product.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="font-body text-body-md text-text-dimmed hover:text-on-surface transition-colors duration-200"
            >
              {link.label}
            </Link>
          ))}
      ...
      <div className="px-4 md:px-margin-desktop max-w-container-max mx-auto border-t border-surface-stroke/30 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <p className="font-body text-body-md text-text-dimmed opacity-80">
